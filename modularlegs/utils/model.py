...
def fix_model_file_path(root):
    parts_dir = os.path.join(LEG_ROOT_DIR, "modularlegs", "sim", "assets", "parts")
...
class XMLCompiler:
    def __init__(self, xml_file):
        ...
        if xml_file.endswith('.xml'):
            if not os.path.exists(xml_file):
                xml_file = os.path.join(LEG_ROOT_DIR, "modularlegs", "sim", "assets", "robots", xml_file)
            tree = etree.parse(xml_file, parser)
    ...
    def reset_hfield(self, radius_x, radius_y, elevation_z, base_z, hfield_file=None, size=None):
        ...
        if hfield_file is not None:
            hfield_file = os.path.join(LEG_ROOT_DIR, "modularlegs", "sim", "assets", "parts", hfield_file)
