...
class RecordVideo(
    ...
    def _capture_frame(self):
        ...
        self.unwrapped_env = self.env.env if hasattr(self.env, "env") else self.env

        viewer = self.unwrapped_env.mujoco_renderer._get_viewer("rgb_array")
        viewer.cam.lookat = self.unwrapped_env.render_lookat_filter(self.unwrapped_env.pos_world)
        viewer.cam.distance = 2

        frame = self.unwrapped_env.render()
    ...
    def reset(
        ...
        if self.episode_trigger and self.episode_trigger(self.episode_id):
            if self.full_name is not None:
                self.start_recording(self.full_name)
            else:
    ...
    def stop_recording(self):
        ...
        if len(self.recorded_frames) == 0:
            ...
            clip.write_videofile(path, logger=moviepy_logger, fps=self.frames_per_sec)
            np_frames = np.array(self.recorded_frames)
            # np.save(path.replace(".mp4", ".npy"), np_frames) # this is huge! # (1001, 480, 480, 3)
            try:
                wandb.log({"Video": wandb.Video(np.transpose(np_frames, (0, 3, 1, 2)), fps=self.frames_per_sec, format="webm")}, commit=False)
            except wandb.errors.Error as e:
                logger.warn(f"Failed to log video to wandb: {e}")
