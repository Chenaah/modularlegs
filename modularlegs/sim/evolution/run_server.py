...
def start_servers(n_servers=18):
    ...
    for i in range(n_servers):
        ...
        command = f"screen -dmS {screen_name} zsh -c 'source $HOME/anaconda3/etc/profile.d/conda.sh && conda activate jax && CUDA_VISIBLE_DEVICES={cuda} python {os.path.join(LEG_ROOT_DIR, 'modularlegs/sim/train_server.py')} {port}; exec zsh'"
...
def start_server(port, cuda, conda="jax"):
    ...
    # command = f"screen -dmS {screen_name} 'source $HOME/anaconda3/etc/profile.d/conda.sh && conda activate {conda} && CUDA_VISIBLE_DEVICES={cuda} MUJOCO_GL=egl python {os.path.join(LEG_ROOT_DIR, 'modularlegs/sim/train_server.py')} {port}; exec zsh'"
    command = f"screen -dmS {screen_name} zsh -c 'source $HOME/anaconda3/etc/profile.d/conda.sh && conda activate {conda} && CUDA_VISIBLE_DEVICES={cuda} MUJOCO_GL=egl python {os.path.join(LEG_ROOT_DIR, 'modularlegs/sim/train_server.py')} {port}; exec zsh'"
    subprocess.run(command, shell=True)
    # os.spawnlp(os.P_NOWAIT, "screen", "screen", "-dmS", screen_name, "zsh", "-c", f"source $HOME/anaconda3/etc/profile.d/conda.sh && conda activate {conda} && CUDA_VISIBLE_DEVICES={cuda} MUJOCO_GL=egl python {os.path.join(LEG_ROOT_DIR, 'modularlegs/sim/train_server.py')} {port}; exec zsh")
    ...
    # python {os.path.join(LEG_ROOT_DIR, 'modularlegs/sim/train_server.py')} {port}" """
