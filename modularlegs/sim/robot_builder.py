...
from modularlegs.utils.math import quat_rotate, quaternion_multiply2, construct_quaternion
from modularlegs.utils.model import create_color_spectrum, fix_model_file_path, vec2string, lighten_color, quaternion_from_vectors
...
class RobotBuilder():
    ...
    def __init__(self, template=None, terrain="flat", mesh_dict=None, robot_cfg=None):
        # Load the template
        if template is None:
            template = os.path.join(LEG_ROOT_DIR, "modularlegs", "sim", "assets", 'template.xml')
    ...
    def save(self, filename="m1"):
        if not filename.endswith(".xml"):
            filename = os.path.join(LEG_ROOT_DIR, "modularlegs", "sim", "assets", "robots", f'{filename}.xml')
...
if __name__ == "__main__":
    ...
    # f = os.path.join(LEG_ROOT_DIR, "modularlegs", "sim", "assets", "robots", f'm1rugged.xml')
    view(f, False)