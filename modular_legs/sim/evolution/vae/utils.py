...
def update_hparams(hparams, model):
    ...
    for k in model.hparams_.keys():
        try:
            if vars(hparams)[k] != model.hparams_[k]:
                print(
                    f"Overriding hparam {k} from {model.hparams_[k]} to {vars(hparams)[k]}"
                )
                model.hparams_[k] = vars(hparams)[k]
        except KeyError:  # not all keys match, it's ok
            pass

    # Add any new hyperparameters
    for k in vars(hparams).keys():
        if k not in model.hparams_.keys():
...
def add_default_trainer_args(parser, default_root=None):
    ...
    pl_trainer_grp.add_argument("--max_epochs", type=int, default=20)
...
def pipe5x1_to_n_modules(pipe):
    """ Converts a 5x1 pipeline to the number of modules """
    return int((len(pipe)-1) / 5)